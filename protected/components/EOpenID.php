 ...
 *
 *
 * Based on Mewp's LightOpenID class (http://gitorious.org/lightopenid)
 * @author CGeorge
 * @license http://www.opensource.org/licenses/mit-license.php MIT
 */
class EOpenID extends CBaseUserIdentity
{
    public $returnUrl
         , $required = array()
         , $optional = array();
         
    private $identity, $claimed_id, $isAuthenticated=false;
    ...
    public function authenticate($identity=null){
        if($identity===null)
            $this->identity = $_POST['openid_identifier'];
        else
            $this->identity=$identity;
        
        Yii::app()->request->redirect($this->authUrl());
